<#
.SYNOPSIS
    Build the Ellipsoid Pattern Generator Windows installer (MSI).
...
.EXAMPLE
    .\installer\build.ps1
    Build release binaries and produce target/installer/Ellipsoid-2.0.0-x64.msi

.EXAMPLE
    .\installer\build.ps1 -SkipBuild -Version 2.1.0
#>
[CmdletBinding()]
param(
    [switch]$SkipBuild,
...
$Binaries = @('ellipsoid-app.exe', 'ellipsoid.exe')
...
Write-Step "Ellipsoid Pattern Generator $cargoVersion (MSI version $msiVersion)"

if (-not $SkipBuild) {
    Write-Step 'Building release binaries'
    Push-Location $RepoDir
    try {
        Invoke-Tool -What 'cargo build' -Exe 'cargo' -Arguments @('build', '--release', '--workspace')
...
foreach ($binary in $Binaries) {
    ...
    if (-not (Test-Path $path)) {
        throw "$path not found. Run without -SkipBuild, or build it first with 'cargo build --release --workspace'."
    }
}
...
$wixObj = Join-Path $objDir 'ellipsoid.wixobj'
$msi = Join-Path $OutDir "Ellipsoid-$cargoVersion-x64.msi"
...
Invoke-Tool -What 'candle' -Exe (Join-Path $wixBin 'candle.exe') -Arguments @(
    ...
    '-out', $wixObj
    (Join-Path $InstallerDir 'ellipsoid.wxs')
)
...
Write-Host ''
Write-Note 'Installs to %LOCALAPPDATA%\Programs\Ellipsoid for the current user; no UAC prompt.'
Write-Note 'Both the app and the `ellipsoid` command-line tool; the folder goes on the user PATH.'
